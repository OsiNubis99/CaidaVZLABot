	...
	if (data.players["p" + playerId].game != null) {
		...
		if (data.games["g" + chatId].player != null) {
			let team = data.games["g" + chatId].players.indexOf(playerId),
				base = [];
			if ((data.games["g" + chatId].cards[team].length == 3) && (data.games["g" + chatId].sings[team].name == "Tres Cartas")) {
				base = [{
						id: 3,
						type: "article",
...
bot.onText(/\/start/, (msg) => {
	bot.sendMessage(msg.chat.id, `Sigue estos pasos: 
...
function crear(msg) {
	...
	if (fake) {
		...
		bot.sendMessage(chatId, "Este bot esta en desarrollo, por favor juega en @JuegaCaidaVZLA\nO solicita a @OsiNubis99 que permita este grupo.");
		bot.getChat(msg.chat.id).then(chat => {
			let add = '';
			if (chat.invite_link)
				add = chat.invite_link;
			bot.sendMessage(groups[0], "Intento de juego en " + msg.chat.title + " " + msg.chat.id + " " + add);
		})
	}
	bd.write(data);
}
	...
	if (data.games["g" + chatId] != null) {
		if (!data.games["g" + chatId].players.find(player => player == msg.from.id)) {
			if (data.games["g" + chatId].player == null) {
				...
				bot.sendMessage(chatId, "Ya la partida inicio, espera que la partida actual termine para poder unirte.");
			}
		} else {
		...
		if (data.games["g" + chatId].player != null) {
			...
			if (data.games["g" + chatId].cards[team].length == 3) {
				if (data.games["g" + chatId].sings[team].name == "Tres Cartas") {
					...
					bot.sendMessage(chatId, "@" + data.players["p" + playerId].username + " cantó " + data.games["g" + chatId].sings[team].name)
				} else {
					bot.sendMessage(chatId, "Ya Cantaste " + data.games["g" + chatId].sings[team].name)
				}
			} else {
...
// jugar - Juega el índice de carta pasado
